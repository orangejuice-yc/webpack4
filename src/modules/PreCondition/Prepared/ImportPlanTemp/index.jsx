...
import { getTmpltaskTree,importPlanTemplate,calendarDefInfo } from '../../../../api/api';
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        render: (text) => {
          if (text) {
            return <span>{dataUtil.Numbers().fomat(dataUtil.WorkTimes().hourTo(text, 'd', this.state.defaultCalendar)) + "D"}</span>
          } else {
            return "--"
          }
        }
