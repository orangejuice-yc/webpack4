...
import { getPreparedTreeList_, getSectionIdsByDefineId } from '../../../api/suzhou-api';
...
export class PlanPrepared extends Component {
  ...
  // 初始化数据
  initDatas = () => {
    dataUtil.CacheOpenProjectByType('predConditionTask').getLastOpenProject((data) => {
   ...
   getPlanTreeList = (projectId = 0,dat = {}) => {
    ...
    if (id != undefined) {
        axios.get(defineTree_(projectId,"4")).then(res => {
            const { data } = res.data
  ...
  getTaskEditAuth = (defineId, taskId, rightData) => {
    axios.get(getTaskEditAuth(defineId || 0, taskId || 0)).then(res => {
      ...
      let edit = status == "EDIT" && auths && auths.indexOf("PRECONDITION-TASK_EDIT") > -1;
      let cprtmEditAuth = auths && auths.indexOf("PRECONDITION-TASK_EDIT") > -1;
      ...
      this.setState({
        ...
        switch (rightData[0].nodeType) {
          ...
          case 'define':
            ...
            if (auths && auths.indexOf("PRED-TASK_EDIT") > -1) {
          ...
          case 'wbs':
            if (auths) {
              ...
              if (auths.indexOf("PRED-TASK_EDIT") > -1 && (status == "EDIT" || status == "CONFIRM")) {
                if (status == "EDIT") {
  ...
  drawcell = () => {
    ...
    gantt.on("drawcell", function (e) {
      ...
      // if (column.name == "org") e.cellHtml = task.org ? "<div title="+task.org.name+">"+task.org.name+"</div>" : ""
      // if (column.name == "user") e.cellHtml = task.user ? "<div title="+task.user.name+">"+task.user.name+"</div>" : ""
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      gantt.render(document.getElementById('preCondition-ganttCt'));
  ...
  openPlan = (selectArray,projectId,projectName) => {
    this.getVariableData( projectId,(data) => {
      this.setState({
        ...
        this.getPreparedTreeList(selectArray,projectId)
      })
    });
  }
  ...
  // 获取计划编制列表
  getPreparedTreeList = (defineIds,projectId) => {
    // 获取查询条件
    ...
    // 将查询条件拼接到URL上，作为参数
    let url = dataUtil.spliceUrlParams(getPreparedTreeList_,{... searchParams});
    // 查询计划编制
    axios.post(url, {projectId:projectId,isPage:false}, null, null, true).then(res => {
     
      let { data } = res.data;
      this.refreshGanttList(data);
      this.setState({ rightData: [] ,defineId : data ?  data[0] ? [data[0].children[0].id]: null : null});
    })
  }
  ...
  render() {
    ...
    return (
      ...
      <ExtLayout renderWidth = {(value) => { this.setContentWidth(value); }}>
        <Toolbar>
          <TopTags
            openProject = {this.openPlan}
            defineOrgTree={this.defineOrgTree} //获取责任主体列表
            orgTree={orgTree}
            projectIds={selectArray}
            defineIds= {this.state.defineId ? this.state.defineId : null}
        ...
        <MainContent contentWidth = {this.state.contentWidth} contentMinWidth = {1500}>
          <div className="miniFit" style={{ height: this.state.ganttHeight, minWidth: 'calc(100vw - 60px)' }}>
            <div ref="ganttCt" id="preCondition-ganttCt" style={{ width: '100%', height: ganttHeight }}></div>
          </div>
        </MainContent>
        <RightTags
          bizType={"task"}
