...
class MenuInfo extends Component {
    ...
    getData = () => {
        let bizType = 'question-record'
        axios.get(fileList(this.props.record.id, bizType)).then(res => {
            this.setState({
                data: res.data.data
            })
        })
    ...
    render() {
        ...
        return (
            <div className={style.main} style={{width:'400px'}}>
                {/* 附件明细 */}
                        ...
                        <div className={style.content}>
                            {
                                this.state.data.map((item, index) => {
                                    return <li key={item.id}>
                                        ...
                                        <span >{"（" + item.size + "）"}</span>
                                        <div className={style.icon}><Icon onClick={this.eyeClick.bind(this, item)} type="eye" style={{ cursor: 'pointer' }} /><Icon type="download" onClick={this.downloadClick.bind(this, item)} style={{ cursor: 'pointer' }} />
                                        </div>
                                    </li>
                                })
                            }
