...
//    this.initDatas();
  ...
  getTaskEditAuth = (defineId, taskId, rightData) => {
    ...
    axios.get(getTaskChangeEditAuth(taskId || 0)).then(res => {
      ...
      let taskChangeReleaseAuth = auths && auths.indexOf("SPECIAL-CHANGE_RELEASE");
      let taskChangeApprovalAuth = auths && auths.indexOf("SPECIAL-CHANGE_RELEASE-APRL");
      ...
      if (rightData.changeType && rightData.changeType.id == "ADD") {
        ...
        cancelEdit = editAuth = status == "EDIT" && auths && auths.indexOf("SPECIAL-CHANGE_EDIT") > -1;
        addEdit = status == "EDIT" && auths && auths.indexOf("CHANGE_ADD_CHILDREN") > -1;
        deleteEdit = false;
      } else if (rightData.changeType && rightData.changeType.id == "DEL") {
        cancelEdit = status == "EDIT" && auths && auths.indexOf("SPECIAL-CHANGE_EDIT") > -1;
        editAuth = false;
        addEdit = false;
        deleteEdit = false;
      } else {
        ...
        deleteEdit = editAuth = taskStatus == "RELEASE" && auths && auths.indexOf("SPECIAL-CHANGE_EDIT") > -1;
      }
  ...
  // 初始化数据
  initDatas = () => {
    dataUtil.CacheOpenProjectByType('specialChange').getLastOpenPlan((data) => {
  ...
  refreshGanttList = (data) => {
    ...
    gantt.unmask();
    gantt.render(document.getElementById('special-ganttCt2'));
      ...
      <ExtLayout renderWidth = {(value) => { this.setContentWidth(value); }}>
        ...
        <MainContent contentWidth = {this.state.contentWidth} contentMinWidth = {1500}>
          <div className="miniFit" style={{ height: this.state.ganttHeight, minWidth: 'calc(100vw - 60px)' }}>
            <div ref="ganttCt2" id="special-ganttCt2" style={{ width: '100%', height: ganttHeight }}></div>
          </div>
        </MainContent>
        <RightTags
