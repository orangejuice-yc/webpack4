...
import { docCorpFolderTree_,menuMenuTree, docCorpFolderAdd_, getDocCorpFolderById_, docCorpFolderUpdate_,docFolderDetete_ } from '../../../../api/suzhou-api'
...
class Manage extends Component {
    ...
    getData = () => {
        axios.post(docCorpFolderTree_ + `?name=${this.props.searchVal ? this.props.searchVal : ''}`).then(res => {
    ...
    addFile = () => {
        ...
        let url = dataUtil.spliceUrlParams(docCorpFolderAdd_);
    ...
    deleteFolder = () => {
        const { record } = this.state
        let url = dataUtil.spliceUrlParams(docFolderDetete_(record.id));
    ...
    upFile = (record, e) => {
        ...
        let url = dataUtil.spliceUrlParams(docCorpFolderUpdate_, { "startContent": "项目【" + this.props.projectName + "】" });
    ...
    menuSelect = () => {
        ...
        if (record) {
            if ("add" == modalType) {
                axios.get(getDocCorpFolderById_(record.id)).then(res => {
                ...
                axios.get(getDocCorpFolderById_(record.parentId == 0 ? record.id : record.parentId)).then(res => {
                    let menuId = res.data.data ? res.data.data.menu ? res.data.data.menu.value : 0 : 0
                    axios.get(menuMenuTree(menuId)).then(res => {
                        this.setState({
                            menuTree: res.data.data
