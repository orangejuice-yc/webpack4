...
class UploadDoc extends Component {
    ...
    handleSubmit = () => {
        ...
        if (!this.state.rightData) {
          dataUtil.message("请选择收藏夹进行操作")
          return ;
        }

        const {startContent}=this.props
        let url = dataUtil.spliceUrlParams(docFavoriteDoc(this.props.record.id, this.state.rightData.id),{startContent});
        axios.post(url, {}, true, '收藏成功',true).then(res => {
            this.props.updateFavorite(this.props.record,1);
            this.handleCancel();
        })
    }
    ...
    addFolder = () => {
        ...
        const {startContent}=this.props
        let url = dataUtil.spliceUrlParams(docFavoriteAdd,{startContent});
        axios.post(url, data, true, '新增成功',true).then(res => {
    ...
    inputBlur = (record, e) => {
        ...
        const {startContent}=this.props
        let url = dataUtil.spliceUrlParams(docFavoriteUpdate,{startContent});
