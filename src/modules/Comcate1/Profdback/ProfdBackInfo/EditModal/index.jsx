...
import{questionHandleInfo,questionHandleUpdate} from '../../../../../api/suzhou-api'
...
class EditModal extends Component {
  ...
  getData = () => {
    axios.get(questionHandleInfo(this.props.record.id)).then(res => {
      this.setState({
        data: res.data.data,
        fileList: res.data.data.files ? res.data.data.files : []
      })
    })
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        let url = dataUtil.spliceUrlParams(questionHandleUpdate,{startContent});
        axios.put(url, body, true, '修改成功',true).then(res => {
  ...
  render() {
    ...
    return (
      <div >
        <Modal title={intl.get('wsd.i18n.comu.profdback.modification')} visible={this.props.visible}
          ...
          footer={
            <div className="modalbtn">
              {/* 关闭 */}
              <Button key={1} onClick={this.handleCancel}>{intl.get('wsd.global.btn.close')}</Button>
              {/* 保存 */}
              <Button key={2} onClick={this.handleSubmit} type="primary">{intl.get('wsd.global.btn.preservation')}</Button>
            </div>
          }
        >
                  ...
                  <Col span={12}>
                    <Form.Item label={intl.get('wsd.i18n.comu.profdback.handlingtime')} {...formItemLayout}>
                      ...
                      {getFieldDecorator('handleTime', {
                        initialValue:   dataUtil.Dates().formatDateMonent(this.state.data.handleTime) ,
                        rules: [{
                          required: true,
                  ...
                  <Col span={12}>
                    <Form.Item label={intl.get('wsd.i18n.comu.profdback.conductor')} {...formItemLayout}>
                      ...
                      {getFieldDecorator('handleUser', {
                        initialValue: this.state.data.handleUser,
                        rules: [],
                      })(
                        <Input />
                      )}
                  ...
                  <Col span={24}>
                    <Form.Item label={intl.get('wsd.i18n.comu.profdback.processingresultdescription')} {...formItemLayout1}>
                      ...
                      {getFieldDecorator('handleResult', {
                        initialValue: this.state.data.handleResult,
                        rules: [{
                          required: true,
