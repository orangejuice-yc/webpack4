...
import { defineTree_ } from '../../../api/suzhou-api'; //隐藏显示列
...
export class PlanPrepared extends Component {
  ...
  // 初始化数据
  initDatas = () => {
    dataUtil.CacheOpenProjectByType('predTask').getLastOpenPlan((data) => {
  ...
  getTaskEditAuth = (defineId, taskId, rightData) => {
    axios.get(getTaskEditAuth(defineId || 0, taskId || 0)).then(res => {
      ...
      let edit = status == "EDIT" && auths && auths.indexOf("PRED-TASK_EDIT") > -1;
      let cprtmEditAuth = auths && auths.indexOf("PRED-TASK_EDIT") > -1;
      let taskReleaseAuth = auths && auths.indexOf("PRED-TASK_RELEASE") > -1;
      let taskCancleReleaseAuth = auths && auths.indexOf("PRED-TASK_RELEASE-CANCEL") > -1;
      let taskApprovalReleaseAuth = auths && auths.indexOf("PRED-TASK_RELEASE-APRL") > -1;
      let taskConfirmAuth = auths && auths.indexOf("PRED-TASK_CONFIRM") > -1;
      let taskCancleConfirmAuth = auths && auths.indexOf("PRED-TASK_CONFIRM-CANCLE") > -1;
      this.setState({
        taskEditAuth: data,
        editAuth: edit,
        cprtmEditAuth,
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      gantt.render(document.getElementById('pred-ganttCt'));
      ...
      <ExtLayout renderWidth = {(value) => { this.setContentWidth(value); }}>
        ...
        <MainContent contentWidth = {this.state.contentWidth} contentMinWidth = {1500}>
          <div className="miniFit" style={{ height: this.state.ganttHeight, minWidth: 'calc(100vw - 60px)' }}>
            <div ref="ganttCt" id="pred-ganttCt" style={{ width: '100%', height: ganttHeight }}></div>
          </div>
        </MainContent>
        <RightTags
          //用于查询流程信息页签
