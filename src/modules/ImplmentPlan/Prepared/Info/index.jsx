      ...
      this.props.form.validateFieldsAndScroll((err, values) => {
        if (!err) {
          ...
          const data = {
            ...
            ...values,
            secutyLevel:'1',
            custom01:this.props.data.custom01,
            custom02:this.props.data.custom02,
          ...
          axios.post(checkTotalDesignWorkload, { ...data }, false, null, true).then(res => {
        ...
        <div className={style.mainScorll}>
          <div className={style.content}>
            {rightData && (<Form onSubmit={this.handleSubmit}>
                ...
                <Col span={12}>
                  <Form.Item label="密级" {...formItemLayout}>
                    {getFieldDecorator('secutyLevel', {
                      ...
                      rules: [{
                        required: true,
                        message: intl.get('wsd.i18n.message.enter') + "密级",
                      }],
                    })(
                      ...
                      <Select>
                        {
                          this.state.info.planType ? 
                             <Option value={this.state.info.planType.id} key={this.state.info.planType.id}>{this.state.info.planType.name}</Option>
                            :
              ...
              {/*任务特有*/}
              {rightData[0]['nodeType'] == 'task' && (
              <Row>
                <Col span={12}>
                  <Form.Item label="设计总量" {...formItemLayout}>
                    {getFieldDecorator('custom03', {
                      initialValue: this.state.info.custom03,
                      rules: [],
                    })(
                      <InputNumber disabled={this.props.menuCode != "ST-IMPLMENT-TASK" } style={{ width: '100%' }} max={999999999999} min={0} onChange={this.onCustom03Change}
                      />
                    )}
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item label="计划完成量" {...formItemLayout}>
                    {getFieldDecorator('custom04', {
                      initialValue: this.state.info.custom04,
                      rules: [],
                    })(
              ...
              <Row>
                <Col span={12}>
                  <Form.Item label="实际完成量" {...formItemLayout}>
                    {getFieldDecorator('custom05', {
                      initialValue: this.state.info.custom05 ? this.state.info.custom05 : 0,
                      rules: [],
                    })(
                ...
                <Col span={12}>
                  <Form.Item
                    ...
                    {getFieldDecorator('custom07', {
                      initialValue: this.props.data.custom07,//this.state.info.custom07,
                      rules: [],
                    })(
                      <Select onFocus={() => {
                        !this.state.commUnit && this.getBaseSelectTree('comm.unit')
                      }}>
                        {
                          this.state.commUnit ? this.state.commUnit.map((v, i) => {
                            return <Option value={v.value} key={i}>{v.title}</Option>
                          }) : this.props.data.custom07 &&
                            <Option value={this.state.info.custom07} key={this.state.info.custom07}>{this.props.data.custom07}</Option>
                        }
                      </Select>
                    )}
                  </Form.Item>
                  ...
                  <Col span={12}> 
                    <Form.Item label={intl.get('wsd.i18n.plan.subTask.custom06')} {...formItemLayout}>
                      {getFieldDecorator('custom06', {
                        initialValue: this.state.info.custom06 && this.state.info.custom06 =='true' ? true : false,
                    ...
                    <Col span={12}>
                      <Form.Item label={intl.get('wsd.i18n.base.planTemAddTask.estWt')} {...formItemLayout}>
                        {getFieldDecorator('estWt', {
                          initialValue: this.state.info.estWt,
                          rules: [{
                            required: true,
