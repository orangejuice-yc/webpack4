...
//    this.initDatas();
    ...
    this.gantt.on("drawcell", function (e) {
      ...
      if (column.name == "custom03") e.cellHtml = task.custom03 ? task.custom03 + custom07 : '0' + custom07
      if (column.name == "custom04") e.cellHtml = task.custom04 ? task.custom04 + custom07 : '0' + custom07
      if (column.name == "custom05") e.cellHtml = task.custom05 ? task.custom05 + custom07 : '0' + custom07
  ...
  findParentTaskId = (task) => {
    ...
    if (task != null) {
      if (task.changeType && task.changeType.id == "ADD") {
        ...
        let parentItem = this.gantt.getParentTask(task)
  ...
  getTaskEditAuth = (defineId, taskId, rightData) => {
    ...
    axios.get(getTaskChangeEditAuth(taskId || 0)).then(res => {
      ...
      let taskChangeReleaseAuth = auths && auths.indexOf("IMPLMENT-CHANGE_RELEASE");
      let taskChangeApprovalAuth = auths && auths.indexOf("IMPLMENT-CHANGE_RELEASE-APRL");
      ...
      if (rightData.changeType && rightData.changeType.id == "ADD") {
        ...
        cancelEdit = editAuth = status == "EDIT" && auths && auths.indexOf("IMPLMENT-CHANGE_EDIT") > -1;
        addEdit = status == "EDIT" && auths && auths.indexOf("CHANGE_ADD_CHILDREN") > -1;
        deleteEdit = false;
      } else if (rightData.changeType && rightData.changeType.id == "DEL") {
        cancelEdit = status == "EDIT" && auths && auths.indexOf("IMPLMENT-CHANGE_EDIT") > -1;
        editAuth = false;
        addEdit = false;
        deleteEdit = false;
      } else {
        ...
        deleteEdit = editAuth = taskStatus == "RELEASE" && auths && auths.indexOf("IMPLMENT-CHANGE_EDIT") > -1;
      }
  ...
  // 初始化数据
  initDatas = () => {
    dataUtil.CacheOpenProjectByType('implmentChange').getLastOpenPlan((data) => {
  ...
  saveGanttByStorage = () => {
    // 设置横道时间刻度
    this.gantt.setTopTimeScale('year') //"year/halfyear/quarter/month/week/day/hour"
    this.gantt.setBottomTimeScale('month')
  ...
  getBaseSelectTree = (typeCode) => {
    axios.get(getBaseSelectTree(typeCode)).then(res => {
      ...
      }else if (typeCode == 'plan.task.planlevel') {// 初始化字典-计划-计划级别
        this.setState({
          planLevelData: data
        })
      }else if (typeCode == 'plan.project.tasktype') {// 初始化字典-任务-作业类型
        this.setState({
          planTaskTypeData: data
        })
      }else if (typeCode == 'plan.project.taskdrtntype') {// 初始化字典-项目-工期类型
        this.setState({
          planTaskDrtnTypeData: data
        })
      }else if (typeCode == 'comm.unit') {// 初始化字典-计量单位
        let map = {};
  ...
  refreshGanttList = (data) => {
    ...
    this.gantt.unmask();
    this.gantt.render(document.getElementById('implment-ganttCt2'));
    this.gantt.setRowHeight(38) //gantt.rowHeight + 25
  ...
  addPlanWbs = (ndata) => {
    if (ndata) {
      ...
      axios.post(url, { ...ndata, parentId, parentType }, true, null, true).then(res => {
        ...
        this.gantt.addTask(data, "add", rightData)
      })
    }
  }
  ...
  addPlanTask = (ndata) => {
    const rightData = this.state.rightData[0];
    const parentTask = this.gantt.getParentTask(rightData)
    let parentId
    let parentType
    ...
    axios.post(url, { ...ndata, parentId, parentType }, true, null, true).then(res => {
      ...
      this.setGanttChangeData(data);
      if (rightData.nodeType == 'task') {
        this.gantt.addTask(data, "before", rightData)
      } else {
        this.gantt.addTask(data, "add", rightData)
      }
    })
  ...
  deletePlanTask = () => {
    ...
    if (rightData && rightData[0]['nodeType'] == 'wbs') {
      ...
      axios.deleted(url, null, true, null, true).then(res => {
        ...
        this.gantt.updateTask(selected, { ...data });
        this.getOnSelectTaskEditAuth();
      })
    }
    if (rightData && rightData[0]['nodeType'] == 'task') {
      ...
      axios.deleted(url, null, true, null, true).then(res => {
        ...
        this.setGanttChangeData(data);
        let selected = this.gantt.getSelected();
        this.gantt.updateTask(selected, { ...data });
        this.getOnSelectTaskEditAuth();
      })
    }
  ...
  updatePlanTask = (ndata) => {
    ...
    axios.put(url, ndata, true, null, true).then(res => {
      let { data } = res.data;
      let commUnitMap=this.state.commUnitMap;
      data["custom07"] = commUnitMap ? commUnitMap[data.custom07]: data.custom07;
      let selected = this.gantt.getSelected();
      this.setGanttChangeData(data);
      this.gantt.updateTask(selected, { ...data });
      this.getOnSelectTaskEditAuth();
    ...
    if (rightData && (rightData[0]['nodeType'] == 'wbs' || rightData[0]['nodeType'] == 'task')) {
      ...
      axios.put(url, null, true, null, true).then(res => {
        ...
        if (rightData[0]['changeType']['id'] == 'ADD') {
          this.gantt.removeTask(selected);
          this.setState({ rightData: null });
        } else {
          this.setGanttChangeData(data);
          this.gantt.updateTask(selected, { ...data, changeId: null, changeType: null });
          this.getOnSelectTaskEditAuth();
        }
      })
    }
      ...
      <ExtLayout renderWidth = {(value) => { this.setContentWidth(value); }}>
        ...
        <MainContent contentWidth = {this.state.contentWidth} contentMinWidth = {1500}>
          <div className="miniFit" style={{ height: this.state.ganttHeight, minWidth: 'calc(100vw - 60px)' }}>
            <div ref="ganttCt2" id="implment-ganttCt2" style={{ width: '100%', height: ganttHeight }}></div>
          </div>
        </MainContent>
        <RightTags
