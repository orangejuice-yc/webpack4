...
import style from '../AttendLog/style.less'
...
class SpecialType extends Component {
    ...
    getList = ()=>{
        const {projectId,sectionId,date,type,search} = this.state;
        let myPaigongdan = JSON.parse(localStorage.getItem("myPaigongdan"));
        if(myPaigongdan || this.state.myPaigongdan){
          // localStorage.removeItem('myPaigongdan')
          axios.get(getDailySheetKqRecord(myPaigongdan.id)).then(res => {
            this.setState({
                ...
                myPaigongdan:myPaigongdan||this.state.myPaigongdan
            });
          });
        }
    }
        ...
        return(
            <div>
                <TopTags
                  ...
                  myPaigongdan = {this.state.myPaigongdan}
                />
