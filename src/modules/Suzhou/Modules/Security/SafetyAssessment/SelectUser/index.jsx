...
import { Table, Modal, Input, Button, notification, Row, Col, Select, Form } from 'antd';
...
import { queryAssessmenter } from '@/modules/Suzhou/api/suzhou-api';
...
class SelectUser extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(getOrgUserTree).then(res => {
      this.setState(
        {
          orgdata: res.data.data.filter(item => item.name === '苏州轨道交通有限公司'),
          initData: res.data.data.filter(item => item.name === '苏州轨道交通有限公司'),
        },
        () => {
  ...
  func = parentId => {
    ...
    let name = '';
    function tree(parentId, arr) {
      for (let i = 0; i < arr.length; i++) {
        if (arr[i].id === parentId && !name) {
          name = arr[i].name;
        } else if (arr[i].children && !name) {
          tree(parentId, arr[i].children);
        }
      }
    }
    tree(parentId, initData);
    return name;
  };
  //确认操作
  handleOk = () => {
    ...
    this.props.form.validateFields((err, value) => {
      if (!err) {
        // const kaoheren = this.state.names.filter(item => item.id === value.kaoheren)[0];
        const arr = rightData.map(item => ({
          bkhrId: item.id,
          bkhrXm: item.name,
          orgId: item.parentId,
        }));
        const params = {
          ...
          // asserId: kaoheren.id,
          // assessmenter: kaoheren.name,
          bkhrAddForms: arr.map(item => {
            return {
              ...item,
              orgName: this.func(item.orgId),
              url:JSON.stringify({url:this.props.menu.url,menuCode:this.props.menu.menuCode,id:item.bkhrId,name:'mySafetyAssessment'})
            };
          }),
        };
  ...
  handleFocus = () => {
    axios.get(queryAssessmenter).then(res => {
      this.setState({ names: res.data.data });
    });
  };
  render() {
      ...
      //左侧表头
      {
        ...
        render: (text, record) => {
          if (record.type == 'org') {
            return (
              <span>
                <MyIcon type="icon-gongsi" style={{ fontSize: 18, vectorEffect: 'middle' }} />
                {text}
              </span>
            );
            ...
            return (
              <span>
                <MyIcon type="icon-yuangong" style={{ fontSize: 18, vectorEffect: 'middle' }} />
                {text}
              </span>
            );
          } else {
            return (
              <span>
                <MyIcon type="icon-bumen1" style={{ fontSize: 18, vectorEffect: 'middle' }} />
                {text}
              </span>
            );
          }
        ...
        >
              ...
              {/* <Col span={7}>
                <Form.Item label="考核人" {...formItemLayout}>
                  {getFieldDecorator('kaoheren', {
                    ...
                    <Select placeholder="请选择" onFocus={this.handleFocus}>
          ...
          <div className={style.box}>
            <Table
              ...
              defaultExpandedRowKeys={['苏州轨道交通有限公司', '机电中心']}
              columns={leftColumns}
              scroll={{ y: 270 }}
            ...
            <div className={style.border}>
              <div>
                <Button
                  onClick={this.moveData.bind(this, 'right')}
                  disabled={this.state.isMove == 'right' ? false : 'disabled'}
                  icon="double-right"
                  style={{ marginBottom: 10 }}
                />
                <Button
                  onClick={this.moveData.bind(this, 'left')}
                  disabled={this.state.isMove == 'left' ? false : 'disabled'}
                  icon="double-left"
                />
              </div>
            </div>
            <Table
              className={style.tableBoxRight}
