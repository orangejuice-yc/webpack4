...
import {addDangerProject,deleteDangerProject} from '@/modules/Suzhou/api/suzhou-api';
...
export class TopTags extends Component{
    ...
    btnClicks = (name, type) => {
        ...
        if(name == 'DeleteTopBtn'){
            ...
            if(deleteArray.length >0){
                axios.deleted(deleteDangerProject, {data:deleteArray}, true).then(res => {
    ...
    submit = (values, type) => {
        ...
        axios.post(addDangerProject, data, true).then(res => {
        ...
        return(
            <div className={style.main}>
                ...
                <div className={style.tabMenu}>
                    ...
                    {props.permission.indexOf('DANGPROACCEPTANCE_EDIT-DANGPROACCEPT')!==-1 && (
                    <PublicButton name={'新增'} title={'新增'} icon={'icon-add'}
                                    afterCallBack={this.btnClicks.bind(this,'AddTopBtn')}
                                    res={'MENU_EDIT'}
                    />)}
                    {props.permission.indexOf('DANGPROACCEPTANCE_RELEASE-DANGPRO')!==-1 && (
                ...
                projectId={this.props.projectId} 
                proc={{  "bizTypeCode": "szxm-dangerProject-approve", "title": this.state.projectName+"安全危大工程验收发布审批" }}
