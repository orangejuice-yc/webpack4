...
import {addRiskRegist,delRiskRegist} from '@/modules/Suzhou/api/suzhou-api';
...
export class TopTags extends Component{
    ...
    btnClicks = (name, type) => {
        ...
        if(name == 'DeleteTopBtn'){
            ...
            if(deleteArray.length >0){
                axios.deleted(delRiskRegist, {data:deleteArray}, true).then(res => {
    ...
    submit = (values, type) => {
        ...
        axios.post(addRiskRegist, data, true).then(res => {
        ...
        return(
            <div className={style.main}>
                <div className={style.search}>
                    <Search search={this.props.search} placeholder={'风险名称'} />
                </div>
                <div className={style.tabMenu}>
                    ...
                    {props.permission.indexOf('RISKREGIST_EDIT-RISKREGIST')!==-1 && ( 
                    <PublicButton name={'新增'} title={'新增'} icon={'icon-add'}
                                    afterCallBack={this.btnClicks.bind(this,'AddTopBtn')}
                                    res={'MENU_EDIT'}
                    />)}
                    {props.permission.indexOf('RISKREGIST_RELEASE-RISKREGIST')!==-1 && ( 
                ...
                projectId={this.props.projectId} 
                proc={{  "bizTypeCode": "szxm-riskRegist-approve", "title": this.state.projectName+"安全风险登记发布审批" }}
