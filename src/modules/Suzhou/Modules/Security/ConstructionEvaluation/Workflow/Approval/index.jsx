...
class PlanPreparedRelease extends Component {
  ...
  initDatas = () => {
    const { projectId, sectionId,year,season } = this.props;
    const params = {
      projectId,
    ...
    axios.get(queryFlowConstructEvaluationList, { params }).then(res => {
      this.setState({
        data: res.data.data,
  ...
  search = text => {
    ...
    let newData = dataUtil.search(initData, [{ key: 'sectionName', value: text }], true);
    this.setState({
      data: newData,
    });
  };
      ...
      <div style={{ display: display }}>
        <div className={style.tableMain}>
          <div className={style.search} style={{ marginTop: '10px', marginBottom: '10px' }}>
            <Search search={this.search} placeholder={'标段名称'} />
          </div>
          <Table
