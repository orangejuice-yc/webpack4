...
import { Icon, Table, Modal, message, notification, Upload } from 'antd';
...
//
import {
  querySecurityExaminationModuleDetailList,
  delSecurityExaminationModuleDetail,
  dowSecurityExaminationModuleDetail,
...
export class RoleList extends Component {
  ...
  getBaseSelectTree = typeCode => {
    axios.get(getBaseSelectTree(typeCode)).then(res => {
      ...
      } else if (typeCode == 'base.professional.type') {
        //专业
        this.setState({
          professional: data,
          professionalMap: map,
  ...
  onClickHandle = name => {
    ...
    if (name == 'DeleteTopBtn') {
      ...
      axios
        .deleted(delSecurityExaminationModuleDetail, { data: selectedRowKeys }, true)
        .then(res => {
          //是不是最后一页
  ...
  export = () => {
    axios.down(dowSecurityExaminationModuleDetail, {
  ...
  render() {
    ...
    const props = {
      name: 'file',
      action: baseURL + '/api/szxm/aqgl/securityExamination/uploadSecurityExaminationModuleDetail',
      headers: {
    ...
    return (
      <div className={style.main}>
        <div className={style.mainHeight}>
          {/* 用户列表 */}
          <h3 className={style.listTitle}> 考核详情 </h3>
          ...
          <div className={style.rightTopTogs}>
            ...
            {this.props.permission.indexOf('TEMPLATE_EDIT-ASSESSDETAIL')!==-1 && (
            <PublicButton
              name={'新增'}
              title={'新增'}
              icon={'icon-add'}
            ...
            <PublicButton
              ...
              content={'你确定要删除吗？'}
            />)}
            {this.props.permission.indexOf('TEMPLATE_IMPORT-SAFETEMPLATE')!==-1 && (
            <PublicButton
              name={'导出模版'}
              title={'导出模版'}
              icon={'icon-iconziyuan2'}
              edit={true}
              afterCallBack={this.export}
            />)}
          ...
          {this.state.addStaffShow && (
            <AddStaffModel
              ...
              professionalMap={this.state.professionalMap}
              education={this.state.education}
              educationMap={this.state.educationMap}
