...
import { Table, Modal, Input, Button,notification,Select,Form } from 'antd';
...
class SelectUser extends React.Component {
  ...
  getInfo = (record, index, type) => {
    ...
    let shoiceData = {...record,isLeader:''};
    ...
    if (type == 'left') {
      ...
      setTimeout(()=>{
       shoiceData.isLeader = this.state.isLeader
      },1)
      this.setState({
        isMove: 'left',
        leftActiveKey: null,
        ...
        var {rightData,choiceData} = this.state
        let index=rightData.findIndex(item=>item.id==choiceData.id)
             rightData[index]=choiceData
      })
    }
  ...
  getOnChange=(value)=>{
      if(value == '是'){
        this.setState({isLeader:'1'})
      }else if(value == '否' ){
        this.setState({isLeader:'0'})
      }   
  }
  //左侧table，点击时添加背景色
