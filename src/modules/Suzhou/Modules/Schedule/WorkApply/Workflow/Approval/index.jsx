...
import { queryFlowWorkApplyList } from '@/modules/Suzhou/api/suzhou-api';
...
class PlanPreparedRelease extends Component {
  ...
  initDatas = () => {
    ...
    axios.get(queryFlowWorkApplyList, { params }).then(res => {
      this.setState({
        data: res.data.data,
  ...
  search = text => {
    ...
    let newData = dataUtil.search(initData, [{ key: 'applyCode', value: text }], true);
    this.setState({
      data: newData,
    });
  };
      ...
      <div style={{ display: display }}>
        <div className={style.tableMain}>
          <div className={style.search} style={{ marginTop: '10px', marginBottom: '10px' }}>
            <Search search={this.search} placeholder={'编号'} />
          </div>
          <Table
