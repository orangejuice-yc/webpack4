...
class Manage extends Component {
    ...
    addFolder = (record) => {
        ...
        axios.post(docProjectFolderAdd, data, true).then(res => {
            // ;
    ...
    //表格 删除文件夹
    deleteFolder = (record) => {
        axios.deleted(docProjectFolderDel(record.id), {}, true).then(res => {
            // 
            let { LeftData, dataMap } = this.state;
    ...
    inputBlur = (record, e) => {
        ...
        axios.put(docProjectFolderUpdate, data, true).then(res => {
                    ...
                    <div className={style.content} onClick={this.DropHandleCancel} style={{ minHeight: '300px' }}>
                        <Row type="flex">
                            <Col span={24}>
                                ...
                                {this.state.LeftData.length != 0 && <Table rowKey={record => record.id} columns={LeftColumns} dataSource={this.state.LeftData} pagination={false}
