...
import { getequipInfo, getequipTypeInfo, updateEquipType, getdictTree, calendarList, updateEquip } from "../../../../api/api"
...
class MenuInfo extends Component {
  ...
  componentDidMount() {
    ...
    //获取设备资源
    if (data.equipType && data.equipType == "equip") {
      axios.get(getequipInfo(data.id)).then(res => {
        if (res.data.data) {
          this.setState({
            info: res.data.data
          })
      ...
      //获取设备类型
      axios.get(getequipTypeInfo(data.id)).then(res => {
        this.setState({
          info: res.data.data
        })
      })
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (data.equipType && data.equipType == "equip") {
          ...
          axios.put(updateEquip, obj, true).then(res => {
          ...
          let obj = {
            ...
            typeCode: values.equipCode,
            typeName: values.equipName
          }
          axios.put(updateEquipType, obj, true).then(res => {
            this.props.updateSuccess(res.data.data)
          })
        }
      }
            ...
            <Col span={12}>
              <Form.Item label={intl.get('wsd.i18n.rsrc.rsrclist.rsrcname')} {...formItemLayout}>
                {getFieldDecorator('equipName', {
                  initialValue: this.state.info.equipName,
                  rules: [{
                    required: true,
            ...
            <Col span={12}>
              <Form.Item label={intl.get('wsd.i18n.rsrc.rsrclist.rsrccode')} {...formItemLayout}>
                {getFieldDecorator('equipCode', {
                  initialValue: this.state.info.equipCode,
                  rules: [{
                    required: true,
